...
void G_BuildTiccmd(ticcmd_t* cmd)
{
	...
	strafe = (gamekeydown[keyboardstrafe] || gamekeydown[keyboardstrafe2]);

	run = ((gamekeydown[keyboardrun] || gamekeydown[keyboardrun2]) ^ alwaysrun);
	...
	if (strafe)
	{
		if (gamekeydown[keyboardright] || gamekeydown[keyboardright2])
			side += sidemove[run];

		if (gamekeydown[keyboardleft] || gamekeydown[keyboardleft2])
			side -= sidemove[run];
	}
	else
	{
		int MaxTurnSpeed = 1000;
		int TurnSpeed = 400;
		int SlowTurnSpeed = 200;
		if (gamekeydown[keyboardright] || gamekeydown[keyboardright2])
		{
			cmd->angleturn = (turnheld < 10)? -SlowTurnSpeed : -TurnSpeed;
	...
	if ((gamekeydown[keyboardjump] || gamekeydown[keyboardjump2]) && !nojump)
		cmd->buttons |= BT_JUMP;

	// buttons
	if (!freeze)
	{
		if (gamekeydown[keyboardfire] || gamekeydown[keyboardfire2])
			cmd->buttons |= BT_ATTACK;

		if (gamekeydown[keyboarduse] || gamekeydown[keyboarduse2])
		{
			cmd->buttons |= BT_USE;
		}
	}
...
{
	// CRITICAL: Reset RNG for deterministic demo playback
	M_ClearRandom();
...
bool G_Responder(const event_t* ev)
{
	...
	if (gameaction == ga_nothing && gamestate == GS_TITLESCREEN)
	{
		...
		else if (!menuactive && !consoleactive && ev->type == ev_keyup
...
void G_Ticker(void)
{
	...
	memcpy(&viewplayer->cmd, &CurrentFrameTicCommands, sizeof(ticcmd_t));
...
{
	// Reset RNG for new game - ensures deterministic demo playback
	M_ClearRandom();
