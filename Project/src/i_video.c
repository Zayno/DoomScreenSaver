...
Rec_InputEvent RecordArray[EPISODE_1_EVENT_COUNT] = {
#include "Episode_1_ProcessedArray.txt"
};


typedef struct
{
	Rec_InputEvent* buffer;
	size_t capacity;
	size_t head;
	size_t tail;
	size_t size;
...
bool Queue_Init(EventQueue* q, size_t capacity)
{
	q->buffer = (Rec_InputEvent*)malloc(sizeof(Rec_InputEvent) * capacity);
	if (!q->buffer)
		return false;
...
bool Queue_Enqueue(EventQueue* q, const Rec_InputEvent* ev)
{
	if (q->size >= q->capacity)
	{
		// Expand capacity (double it, or start with 16 if zero)
		size_t newCapacity = q->capacity ? q->capacity * 2 : 16;
		Rec_InputEvent* newBuffer = (Rec_InputEvent*)malloc(sizeof(Rec_InputEvent) * newCapacity);
		if (!newBuffer)
...
bool Queue_Dequeue(EventQueue* q, Rec_InputEvent* out)
{
	if (q->size == 0)
		return false; // empty
...
bool Queue_Peek(const EventQueue* q, Rec_InputEvent* out)
{
	if (q->size == 0)
		return false;
...
bool LoadEventsFromStaticArray(EventQueue* q)
{
	for (size_t i = 0; i < EPISODE_1_EVENT_COUNT; i++)
	{
		Rec_InputEvent ev = { 0 };
		ev.timestamp = RecordArray[i].timestamp;
		ev.isKeyDown = (RecordArray[i].isKeyDown == 0) ? 0 : 1;
		ev.KeyData = RecordArray[i].KeyData;

		if (!Queue_Enqueue(q, &ev))
		{
			return false; // should never happen if sized correctly
		}
...
static void I_GetEvent(void)
{
	...
	Rec_InputEvent NextEvent = { 0 };
	bool hasEvent = Queue_Peek(&ReplayQ, &NextEvent);

	// Use exact frame matching for deterministic replay
	if (hasEvent && MainLoopTics == NextEvent.timestamp)
	{
		HaveInput = true;
	}

	// Process ALL replay events for the current frame before processing SDL events
	while (HaveInput)
	{
		event_t     ev = { 0 };
		ev.data1 = NextEvent.KeyData;
		Queue_Dequeue(&ReplayQ, NULL);

		if (NextEvent.isKeyDown)
		{
			ev.type = ev_keydown;
			ev.data2 = 0;
			D_PostEvent(&ev);
		...
		// Check if there are more events for this exact frame
		hasEvent = Queue_Peek(&ReplayQ, &NextEvent);
		HaveInput = (hasEvent && MainLoopTics == NextEvent.timestamp);
	}
	...
	while (SDL_PollEvent(Event))
	{
		...
		// During replay, ignore real input (we already processed replay events above)
		// But still handle window events
...
static void I_Blit_Shake(void)
{
	...
	dest_rect.x += M_VisualRandomInt(-10, 10);
	dest_rect.y += M_VisualRandomInt(-10, 10);
...
static void I_Blit_NearestLinear_Shake(void)
{
	...
	dest_rect.x += M_VisualRandomInt(-2, 2);
	dest_rect.y += M_VisualRandomInt(-2, 2);
...
void I_UpdateBlitFunc(const bool shaking)
{
	...
	{
		if (shaking && !software)
			blitfunc = &I_Blit_NearestLinear_Shake;
		else
			blitfunc =  &I_Blit_NearestLinear;
...
//#endif
	...
	if (!Queue_Init(&ReplayQ, EPISODE_1_EVENT_COUNT))
	{
		return;
	}
	LoadEventsFromStaticArray(&ReplayQ);
