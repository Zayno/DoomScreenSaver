...
{
    // Stop the MIDI stream first - silences all notes
    if (hMidiStream)
    {
    ...
    // Kill the player thread immediately - it's just sitting in a wait loop
    // and we've already reset the MIDI device so there's nothing to clean up
    if (hPlayerThread)
    {
        TerminateThread(hPlayerThread, 0);
